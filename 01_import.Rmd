...
low_data <- cbind(
  low_plot_oct,
  extract(low_apr21, vect(low_plot_apr), fun = mean, na.rm = TRUE) %>% 
    dplyr::select(-ID) %>% 
    setNames(paste0(names(.), "_apr21")),
  extract(low_oct21, vect(low_plot_oct), fun = mean, na.rm = TRUE) %>% 
    dplyr::select(-ID) %>% 
    setNames(paste0(names(.), "_oct21")))

mid_data <- cbind(
  mid_plot_oct,
  extract(mid_apr21, vect(mid_plot_apr), fun = mean, na.rm = TRUE) %>% 
    dplyr::select(-ID) %>% 
    setNames(paste0(names(.), "_apr21")),
  extract(mid_apr21, vect(mid_plot_oct), fun = mean, na.rm = TRUE) %>% 
    dplyr::select(-ID) %>% 
    setNames(paste0(names(.), "_oct21")))

upp_data <- cbind(
  upp_plot_oct,
  extract(upp_apr21, vect(upp_plot_apr), fun = mean, na.rm = TRUE) %>% 
    dplyr::select(-ID) %>% 
    setNames(paste0(names(.), "_apr21")),
  extract(upp_oct21, vect(upp_plot_oct), fun = mean, na.rm = TRUE) %>% 
    dplyr::select(-ID) %>% 
    setNames(paste0(names(.), "_oct21")))
